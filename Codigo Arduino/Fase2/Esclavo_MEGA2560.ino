...
// ✅ CAMBIO: guardar UID anterior para detectar "tarjeta nueva"
byte lastUid[10];
byte lastUidSize = 0;
...
int calcularIntervalo(int distancia) {
  ...
  if (distancia > 30) return 800;
  if (distancia > DISTANCIA_PARADA) return 300;
  return 0;
}
...
void setMotorSpeeds(int v1, int v2, int v3, int v4, bool parada) {
  ...
  if (v1 >= 0) { digitalWrite(motor1A, HIGH); digitalWrite(motor1B, LOW); }
  else { digitalWrite(motor1A, LOW); digitalWrite(motor1B, HIGH); v1 = -v1; }

  if (v2 >= 0) { digitalWrite(motor2A, HIGH); digitalWrite(motor2B, LOW); }
  else { digitalWrite(motor2A, LOW); digitalWrite(motor2B, HIGH); v2 = -v2; }

  if (v3 >= 0) { digitalWrite(motor3A, HIGH); digitalWrite(motor3B, LOW); }
  else { digitalWrite(motor3A, LOW); digitalWrite(motor3B, HIGH); v3 = -v3; }

  if (v4 >= 0) { digitalWrite(motor4A, HIGH); digitalWrite(motor4B, LOW); }
  else { digitalWrite(motor4A, LOW); digitalWrite(motor4B, HIGH); v4 = -v4; }
...
// ✅ CAMBIO: comparar UID actual con UID anterior
bool isNewCardUID() {
  if (!hasLastUid) return true;
  if (mfrc522.uid.size != lastUidSize) return true;
  for (byte i = 0; i < mfrc522.uid.size; i++) {
    if (mfrc522.uid.uidByte[i] != lastUid[i]) return true;
  }
  return false;
}

// ✅ CAMBIO: guardar UID actual como "última"
void saveCurrentUID() {
...
void RFID() {
  ...
  // ✅ CAMBIO: si la UID es nueva, cambiamos zona (por RFID)
  if (isNewCardUID()) {
    zonaActual = (zonaActual + 1) % 4;   // 0->1->2->3->0
    saveCurrentUID();

    // Forzar re-envío al cambiar de zona
    tarjetaDetectada = false;
    lastZonaSent = -1;

    Serial.print("✅ ZONA CAMBIA por RFID -> ");
  ...
  if (mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, bloque, &key, &(mfrc522.uid)) != MFRC522::STATUS_OK) {
    mfrc522.PICC_HaltA();
